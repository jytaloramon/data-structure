...
int main(int argc, char const *argv[]){
    ...
    int op, v1, v2, rs,
        ...
        segt_arr[LEN * SEGMULTI];

    // Setting all array positions to 0
    memset(segt_arr, 0, sizeof(int) * LEN * SEGMULTI);

    SegTree *seg_tree = segtree_new(arr, segt_arr, (CoordCartesian){LEN});
    ...
    for (int i = 0; i < INPUTSIZE; ++i){
        ...
        switch (op){
        case SUMBETWEEN:
            rs = segtree_sum_between(seg_tree, (CoordCartesian){v1}, (CoordCartesian){v2});
            printf(" * SUMBETWEEN [%d - %d]: %d\n\n", v1, v2 - 1, rs);
            break;
        case UPDATE:
            printf(" * UPDATE [%d](%d) to (%d)\n\n", v1, seg_tree->arr[v1], v2);
            segtree_update(seg_tree, v2, (CoordCartesian){v1});
            segtree_show(seg_tree);
        }
    }
...
void segtree_show(SegTree *seg_tree){
    ...
    for (j = seg_tree->shape.col * SEGMULTI - 1; j >= 0 && seg_tree->segt_arr[j] == 0; --j);

    printf("\nSegtree_arr: ");
    for (i = 0; i < j + 1; ++i)
        printf("%d, ", seg_tree->segt_arr[i]);
