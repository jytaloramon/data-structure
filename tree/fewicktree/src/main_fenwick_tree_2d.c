...
int main(int argc, char const *argv[]){
    ...
    int test_cases[INPUTSIZE][5] = {
        {SUMBETWEEN, 0, 0, 1, 2},
        {SUMBETWEEN, 0, 0, LEN, LEN},
        {UPDATE, 0, 0, 10, 0},
        {SUMBETWEEN, 0, 0, 1, 2},
        {UPDATE, 3, 3, 8, 0},
        {SUMBETWEEN, 0, 0, LEN, LEN},
        {SUM, 3, 3, 0, 0},
        {SUMBETWEEN, 2, 2, LEN, LEN},
        {UPDATE, 2, 3, 9, 0},
        {SUMBETWEEN, 1, 3, LEN, LEN},
    };
    ...
    FenwickStructure *fwks = bitree_new(array, array_bit, (CoordCartesian){LEN, LEN});
    ...
    for (int i = 0; i < INPUTSIZE; ++i){
        op = test_cases[i][0]; v[0] = test_cases[i][1];
        v[1] = test_cases[i][2]; v[2] = test_cases[i][3];
        v[3] = test_cases[i][4];
        ...
        case SUM:
            ...
            break;
        case SUMBETWEEN:
            rs = bitree_sum_between(fwks, (CoordCartesian){v[0], v[1]}, (CoordCartesian){v[2], v[3]});
            printf(" * SUMBETWEEN [(%d, %d) - (%d, %d)]: %d\n\n", v[0], v[1], v[2] - 1, v[3]- 1, rs);
            break;
        case UPDATE:
            printf(" * UPDATE [%d, %d](%d) to (%d)\n\n", v[0], v[1], fwks->arr[v[0]][v[1]], v[2]);
