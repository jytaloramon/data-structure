...
size_t quick_partition(void *arr, size_t n_member, size_t size_m, ICOMPARATOR) {
    ...
    char *arr_c = (char *)arr;
    size_t i = 1, j = n_member - 1;

    while (i < j) {

        while (i < j &&
               comparator(arr_c + SHIFTBYTEIDXSIZEM(i, size_m), arr_c) <= 0) {
            i++;
        }
    ...
    if (comparator(arr_c + SHIFTBYTEIDXSIZEM(j, size_m), arr_c) < 0)
        SWAPVARIABLE(arr_c, arr_c + SHIFTBYTEIDXSIZEM(j, size_m), size_m);
