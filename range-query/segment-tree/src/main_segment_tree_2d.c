...
int main(int argc, char const *argv[]) {
    ...
    int test_cases[INPUTSIZE][6] = {
        {SUMBETWEEN, 0, 0, LEN, LEN, 0}, {SUMBETWEEN, 0, 1, 4, 3, 0},
        {UPDATE, 2, 2, 25, 0, 0},        {SUMBETWEEN, 3, 3, LEN, LEN, 0},
        {SUMBETWEEN, 2, 3, 3, 4, 0},     {SUMBETWEEN, 1, 1, 3, 4, 0},
        {SUMBETWEEN, 0, 3, 4, 4, 0},     {UPDATE, 3, 3, 31, 0, 0},
        {UPDATE, 0, 0, 0, 0, 0},         {SUMBETWEEN, 0, 0, 3, 3, 0},
        {UPDATE, 2, 3, 11, 0, 0},        {UPDATE, 1, 3, 4, 0, 0},
    ...
    int op, *v, rs;
    int arr[LEN][LEN] = {
        {1, 2, 3, 4}, {5, 6, 7, 8}, {1, 7, 5, 9}, {3, 0, 6, 2}};
    ...
    SegTree2d *seg_tree2d = segtree2d_new((int *)arr, (int *)arr_segt,
                                          (CoordCartesian2D){LEN, LEN});
    ...
    for (int i = 0; i < INPUTSIZE; ++i) {
        ...
        switch (op) {
        case SUMBETWEEN:
            rs = segtree2d_sum_between(seg_tree2d,
                                       (CoordCartesian2D){v[0], v[1]},
                                       (CoordCartesian2D){v[2], v[3]});
            printf(" * SUMBETWEEN [(%d, %d) - (%d, %d)]: %d\n\n", v[0], v[1],
                   v[2] - 1, v[3] - 1, rs);
            break;
        case UPDATE:
            printf(" * UPDATE [%d, %d](%d) to (%d)\n\n", v[0], v[1],
                   ((int(*)[LEN])seg_tree2d->arr)[v[0]][v[1]], v[2]);
...
void segtree_show(SegTree2d *seg_tree) {
    ...
    int len = seg_tree->shape.col, (*arr_link)[len] = (int(*)[])seg_tree->arr,
